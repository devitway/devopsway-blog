...
В феврале Qwen Code выглядел как бесплатная альтернатива Claude Code. 15 апреля Alibaba [закрыла бесплатный доступ](https://github.com/QwenLM/qwen-code/issues/3203). Инструмент устарел за два месяца.
...
[Aider](https://aider.chat/) – 30K+ звёзд, Python, заточен под работу с git-репозиториями. Строит структурную карту кодовой базы, автоматически запускает линтер и тесты после каждого изменения.
...
**Правило:** перед установкой проверяйте на [ollama.com/search?c=tools](https://ollama.com/search?c=tools), есть ли у модели тег `tools` в capabilities. Но тег – необходимое, но не достаточное условие: `phi4-mini` имеет тег `tools`, а tool calls всё равно сломаны.
