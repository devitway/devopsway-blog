...
Про это — [День 2](/posts/day-02-reset-recovery/) (reflog + reset). Сейчас важно: **committed** — это точка «назад без потерь только через reflog».
...
Больше типов и автоматическая проверка через `commitlint` — в [Дне 5](/posts/day-05-hooks/) (hooks).
...
- **[День 2](/posts/day-02-reset-recovery/)** → reflog и fsck: что спасает после `git reset --hard`, а что нет. Три состояния под углом восстановления
...
1. Одна строка с двумя индикаторами:
   ```
   MM README.md
   ```
   Первый `M` (колонка staging) — файл изменён и добавлен в staging. Второй `M` (колонка working) — файл изменён после `git add`. В коммит пойдёт только staged-версия, а новые правки останутся в working.
