...
Значит нужен простой вход: открыл браузер, залогинился как в гугл, подтвердил вторым фактором – и ты внутри. Без клиента, без профилей, без звонков в поддержку.
...
Взял Zitadel. Он на Go – лёгкий бинарь, быстрый старт, скромный по памяти. OIDC из коробки, организации, Actions (кастомная логика на входе), нормальная консоль. Поднялся, стал единым стором личностей: один аккаунт на все тулы, 2FA, аудит входов в одном месте.
...
```
браузер → grafana.company.ru → Pomerium → (нет сессии) Zitadel + 2FA → назад → апстрим
```

Первый раз тебя редиректит в Zitadel, логин плюс второй фактор, дальше сессия живёт и по остальным тулам вход молчаливый – та же сессия, пароль второй раз не спрашивают.
...
На выходе: один вход с 2FA на весь зоопарк, deny-by-default перед каждой тулой, онбординг и оффбординг через MR, аудит в одном месте. Маркетолог заходит из браузера и не знает слов "VPN-профиль". Инженер логинится один раз на все тулы.
