...
**1997 – HTTP/1.1 (RFC 2068).** Keep-alive (постоянное соединение): одно TCP-соединение для нескольких запросов. Заголовок `Host`: виртуальные хосты. До сих пор используется.

**1994 – SSL (Netscape).** Kipp Hickman разрабатывает Secure Sockets Layer для безопасной передачи кредитных карт. SSL 2.0 → SSL 3.0 → TLS 1.0 (1999) → TLS 1.2 (2008) → **TLS 1.3 (2018)** – текущий стандарт.
...
**2022 – HTTP/3 (RFC 9114).** Поверх QUIC (UDP), а не TCP. Установка соединения без предварительного кругового обмена (Zero-RTT).
...
> **Идемпотентность** = повторный запрос даёт тот же результат. GET `/users/42` дважды → одно и то же. POST `/users` дважды → два пользователя. Это важно для логики повторов (retry) и проектирования API.
...
# Игнорировать невалидный сертификат (ТОЛЬКО для отладки!):
...
```
Клиент → nginx (reverse proxy) → backend (app)
...
**На собесе:** "502 – nginx не смог получить валидный ответ от апстрима (upstream). Причины: бэкенд упал, не слушает, файрвол блокирует, OOM-kill. Диагностика: curl к бэкенду напрямую (мимо nginx), проверка ss/netstat на бэкенде, логи nginx error.log."
...
Ты владеешь HTTP: методы, коды, заголовки, curl для диагностики. Понимаешь TLS: рукопожатие (handshake), сертификаты, openssl для проверки. Это "прикладной" уровень сетей.
