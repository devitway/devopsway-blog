...
**2. Вектор** -- координаты смысла в многомерном пространстве. Модель-эмбеддер превращает текст в массив чисел фиксированной длины. Тексты с похожим значением получают близкие координаты, даже если написаны разными словами:
...
```
cosine("Docker container", "containerization") = 0.68  ← похожи
cosine("Docker container", "borsch recipe")    = 0.05  ← не похожи
```

Чем ближе к единице -- тем больше общего в значении, даже если сами слова разные. 0 -- ничего общего, 1 -- одно и то же. На практике score выше 0.7 -- хорошее совпадение.

> **Важно:** это score на английском тексте с моделью all-MiniLM. С русским всё сложнее: embedding-модели обучены преимущественно на английском, и русские фразы получают значительно более низкие score. Подробнее -- в [RAG Pipeline 2/N: Embeddings](/posts/rag-02-embeddings/).
...
"как настроить proxy"         ┌──────────────────┐
 ...
 [0.11, -0.31, ...]       косинусная близость
        ...
        └──────────────────────────────┘
                    ...
                    ▼
            Ранжирование:
            1. nginx-guide.md  → 0.82
            2. docker-guide.md → 0.35
            3. ssh-guide.md    → 0.12
                    │
                    ▼
...
| Размерность | 384 (all-MiniLM) | Быстрое получение вектора, достаточное качество |
