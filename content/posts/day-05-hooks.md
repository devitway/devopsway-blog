...
**Главная мысль:** hooks — это **дружеская страховка на вашем компе**, не жёсткая защита сервера. Кто очень хочет — обойдёт через `git commit --no-verify`. Серьёзная защита делается в CI и серверных hooks (pre-receive на GitLab/GitHub). Hooks на клиенте ловят то, что **забыли**, а не то, что **пытались скрыть**.
