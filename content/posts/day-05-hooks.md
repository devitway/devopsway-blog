...
Это не заменяет CI, но экономит минуты: падающие тесты ловятся на своей машине, а не после push и жёлтого прогресс-бара в GitLab Actions.
...
- **Challenge** → `docker run devitway/git-challenge`: одна из задач — починить репо, где установлен вредный commit-msg hook, блокирующий все коммиты
