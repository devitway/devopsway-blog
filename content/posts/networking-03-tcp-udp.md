...
| | TCP | UDP |
|---|---|---|
| **Соединение** | Да (рукопожатие, handshake) | Нет (без установления, fire-and-forget) |
...
| **Накладные расходы (overhead)** | Высокий (20+ байт заголовок) | Низкий (8 байт) |
...
"Connection timeout" (ожидание 30+ секунд):
     ...
     │                         │  - файрвол с DROP (не REJECT)
     │                         │  - Хост выключен
     │                         │  - Нет маршрута
```

> **20/80 для диагностики (troubleshooting):**
> - **Connection refused** → сервис не запущен ИЛИ слушает на другом порту/интерфейсе
> - **Connection timeout** → файрвол DROP, хост недоступен, неправильный IP/порт
...
Строго говоря, "системные" порты (well-known) – это только 0–1023 (ниже: SSH, DNS, HTTP, HTTPS). Остальные – зарегистрированные пользовательские, но в DevOps встречаются постоянно:
...
### Эфемерные порты (ephemeral, 32768–60999)
...
# В TCP-пакете:
# src_port = 54321 (эфемерный – случайный)
# dst_port = 8080  (системный – целевой)
...
## Сокет – и почему `ss` так называется

`ss` расшифровывается как **socket statistics**. А что такое сокет – ещё помнишь? В сетях сокет – это конечная точка соединения, пара `IP:порт`.

Слушающий сокет (listening) один: `0.0.0.0:8080`. А вот установленное соединение задаётся уже **четвёркой** (4-tuple):

```
(IP:порт клиента)  ↔  (IP:порт сервера)
 10.0.0.10:54321   ↔   10.0.0.5:8080
```
...
- **Unix-сокет** (Unix domain socket) – не `IP:порт`, а файл: `/run/postgresql/.s.PGSQL.5432`, `/run/docker.sock`. Тот же PostgreSQL слушает и сетевой сокет `IP:5432`, и локальный файл-сокет (`ss -x` покажет их).
- **Процессорный сокет** (CPU socket из `lscpu`) – это железо, разъём под процессор.
- **Вызов `socket()`** из Berkeley API – программистская абстракция создания сокета; под капот приложения нам не надо.
...
> **netstat vs ss:** `netstat` – пакет `net-tools` (устаревший, deprecated). `ss` – пакет `iproute2` (современный, быстрее). На собесе используй `ss`.
...
Двустороннее (сломанное):
  Клиент → SYN → Сервер       ✓ Сервер знает о клиенте
  Клиент ← SYN-ACK ← Сервер   ✓ Клиент знает о сервере
  Но сервер НЕ знает, получил ли клиент SYN-ACK!

Трёхстороннее (рабочее):
  Клиент → SYN → Сервер       ✓ Сервер знает о клиенте
  Клиент ← SYN-ACK ← Сервер   ✓ Клиент знает о сервере
  Клиент → ACK → Сервер       ✓ Сервер знает, что клиент получил
```

**На собесе:** "Трёхстороннее рукопожатие гарантирует, что обе стороны синхронизировали начальные порядковые номера (sequence numbers) и обе подтвердили готовность. Без третьего ACK – полуоткрытые соединения (half-open) и утечка ресурсов на сервере (атака SYN-флуд эксплуатирует именно это)."
...
3. **Видео/аудио** – лучше пропустить кадр, чем ждать повторной передачи (TCP retransmit = задержка)
4. **Игры** – латентность > надёжность
5. **QUIC/HTTP3** – Google построил надёжный протокол ПОВЕРХ UDP (вместо TCP), потому что TCP-рукопожатие = лишний круговой обмен (RTT)

**На собесе:** "UDP выбирают, когда потеря отдельных пакетов допустима, а задержка – нет. Или когда обмен запрос-ответ (request-response) укладывается в один пакет (DNS). Современный тренд – QUIC: надёжность TCP + скорость UDP, потому что рукопожатие совмещено с TLS-рукопожатием (handshake)."
...
# 2. Активные соединения к PostgreSQL (на сервере БД порт 5432 – локальный):
ss -tnp src :5432 | grep ESTAB | wc -l
# С клиента, который сам ходит в БД, наоборот: dst :5432
