...
Слушающий сокет один: `0.0.0.0:8080`. А вот установленное соединение задаётся уже **четвёркой** (4-tuple):
...
**И не путать:** есть ещё **Unix-сокет** – не `IP:порт`, а файл: `/run/postgresql/.s.PGSQL.5432`, `/run/docker.sock`. Тот же PostgreSQL слушает и сетевой сокет `IP:5432`, и локальный файл-сокет (`ss -x` покажет их).
...
Полный разбор с пакетами – выше, в разделе про рукопожатие. Коротко:

**На собесе:** "Refused – RST пришёл, хост жив, но порт закрыт (проблема на хосте: сервис). Timeout – ответа нет, пакет не дошёл или заблокирован (проблема в сети: маршрутизация/firewall). Refused мгновенный, timeout висит 30–120 секунд."
...
**На собесе:** "Три пакета синхронизируют начальные порядковые номера (sequence numbers) и подтверждают готовность обеих сторон. Без третьего ACK – полуоткрытые соединения (half-open) и утечка ресурсов на сервере, именно это эксплуатирует SYN-флуд."
...
**На собесе:** "UDP выбирают, когда потеря отдельных пакетов допустима, а задержка – нет. Или когда обмен запрос-ответ укладывается в один пакет (DNS). Современный тренд – QUIC: надёжность TCP + скорость UDP, рукопожатие совмещено с TLS."
