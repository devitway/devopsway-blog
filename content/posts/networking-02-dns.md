...
| **PTR** | Обратная запись (IP → имя) | `34.216.184.93 → api.example.com` | Обратный DNS |
...
> **20/80:** используй `dig`, не `nslookup`. `dig` показывает TTL, раздел авторитетных серверов (authority section), время запроса – всё, что нужно для диагностики. `nslookup` устарел, но знай его для собеса.
...
dig +noall +answer api.example.com   # TTL – второй столбец
...
**Ограничения CNAME:**
1. CNAME **нельзя** ставить на корень домена (apex, `example.com` без www). Только на поддомены. Причина: CNAME конфликтует с записями SOA и NS на корне.
2. CNAME – дополнительный DNS-запрос, а значит задержка

**Альтернатива:** AWS Route53 и Cloudflare поддерживают ALIAS/ANAME – работают как CNAME, но на корне.

**На собесе:** A напрямую отдаёт IP, CNAME гоняет на другое имя (лишний lookup) и не живёт на корне; балансировка – несколько A-записей (round-robin) или ALIAS.

---

### Подвох 2: "Почему после смены DNS-записи часть пользователей видит старый адрес?"

**Ответ:** TTL кеширования (механику миграции и снижение TTL см. в разделе TTL выше).

DNS – система с согласованностью в конечном счёте (**eventually consistent**). Запись кешируется на TTL секунд на каждом уровне независимо:
- В резолвере (8.8.8.8): до TTL
- В ОС: до TTL (или перезагрузки)
- В браузере: свой кеш ~60 секунд (Chrome – независимо от TTL записи)
- В Java: по умолчанию 30 секунд; "вечно" – только со старым SecurityManager (`networkaddress.cache.ttl=30` в политике безопасности)

**На собесе:** кеш живёт на каждом уровне – резолвер, ОС, приложение – и держится TTL секунд; после смены записи либо ждёшь TTL, либо снижаешь его заранее. Отдельная граната – JVM: без него дефолт 30 секунд, а со старым SecurityManager кешировал вечно.
...
**На собесе:** кеш доживает до TTL, новые запросы отваливаются по таймауту – поэтому в проде всегда два nameserver в resolv.conf, а в K8s CoreDNS в нескольких репликах.
