...
- В Java: по умолчанию 30 секунд; "вечно" – только со старым SecurityManager
...
**На собесе:** "DNS кешируется на каждом уровне: резолвер (resolver), ОС, приложение. TTL определяет, как долго кеш жив. После смены записи нужно ждать TTL – или заранее снизить его. Особый случай – JVM, который при устаревшем SecurityManager кешировал DNS вечно (без него дефолт – 30 секунд): нужен `networkaddress.cache.ttl=30` в политике безопасности (security policy)."
