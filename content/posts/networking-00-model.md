...
**1984 – OSI модель.** ISO (Международная организация по стандартизации) публикует "идеальную" семиуровневую модель. К этому моменту TCP/IP разрабатывается уже 10 лет (ARPANET перешёл на него в 1983). OSI – теоретическая модель, TCP/IP – практическая. Победил практик, но модель OSI осталась как **язык для описания** сетевых проблем.
...
| 6 | Presentation | SSL/TLS (на практике L5-6), сжатие, кодировки | –    |
...
Каждый уровень оборачивает данные предыдущего в свой заголовок – как конверт в конверте. Получатель **распаковывает в обратном порядке**: Ethernet → IP → TCP → HTTP.
...
1. DNS (Уровень 7): браузер резолвит example.com → 93.184.216.34
   └─ Проверяет: кеш браузера → /etc/hosts → кеш ОС → DNS-сервер
...
> **20/80:** на собесе достаточно уверенно пройти шаги 1–6. Шаг 7 (рендеринг) – зона фронтендера.
...
ping -c 4 8.8.8.8
# PING 8.8.8.8: 64 bytes from 8.8.8.8: icmp_seq=1 ttl=54 time=1.23 ms

# Что значит вывод:
# 64 bytes    – размер ответа
# ttl=54     – Time To Live (сколько маршрутизаторов осталось; у Linux начинал с 64, прошёл 10)
...
# Вариант через ICMP Echo вместо UDP (проходит там, где UDP-пробы режут):
traceroute -I 8.8.8.8

# Непрерывный вариант (параллельные пробы):
...
**Системное мышление:** OSI – это модель, абстракция для понимания. TCP/IP – реализация, код, который работает. Знай модель, работай с реализацией.
...
**На собесе:** "Switch – по MAC внутри сети, router – по IP между сетями. И тут добавь то, чего от тебя не ждут: домашний "роутер" – это router, switch и Wi-Fi access point в одном корпусе, в data center они разделены."
...
**На собесе:** "127.0.0.1 виден только локально, 0.0.0.0 – с любого IP хоста. Дальше – грабли: приложение слушает на 127.0.0.1 внутри Docker-контейнера, снаружи не достучаться, потому что контейнер – отдельный network namespace."
