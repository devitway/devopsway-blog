...
**Главный вывод:** ни одна из моделей не даёт на русском тексте score выше 0.8 для семантически идентичных фраз. На английском all-MiniLM выдаёт 0.68 для "Docker container" / "containerization" – тоже не блестяще, но мусор получает заметно более низкий score. На русском BPE-tokenizer разбивает слово на ~5 subword-частей (не побуквенно, но значительно дороже английского: ~5 tok/word vs ~1.6 tok/word), и модель теряет контекст – это не "цена мультиязычности", а следствие того, что русского текста в обучающих данных было мало.
...
### 1. Токенизация: русское слово = ~5 токенов

Embedding-модели используют BPE-tokenizer, обученный преимущественно на английском тексте. Кириллица не попала в частотные пары, поэтому дробится на byte-level фрагменты.

Эмпирический тест на mxbai-embed-large (512 tokens max) – заполняем окно реальным техническим текстом и считаем, сколько влезает:

```
512 токенов вмещают:
  Русский технический текст: ~700-760 символов (~105 слов, ~4.8 tok/word)
  Английский аналог:        ~2100-2500 символов (~320 слов, ~1.6 tok/word)
  Разница:                   ~3x по символам, ~7x по tokens/word
```

Русское слово в **связном тексте** – ~5 токенов (не 1:1 побуквенно, BPE находит некоторые частотные пары в потоке). Английское слово – ~1-2 токена.

Последствие: русский текст длиной 800 символов – это ~530 токенов, уже на грани лимита mxbai-embed-large (512). Модель с контекстом 256 токенов (all-MiniLM) обрежет его молча, потеряв больше половины.
...
800 символов русского текста ≈ 530 токенов (одно русское слово ≈ 5 токенов в связном тексте – BPE-дробление кириллицы). Это уже на грани лимита mxbai-embed-large (512 tokens). Модель с контекстом 256 (all-MiniLM) потеряет больше половины. Решение: обрезать текст перед отправкой (safe_embed) или уменьшить размер чанка.
...
| Объём | 206,000+ векторов | Техническая документация + рабочие заметки |
