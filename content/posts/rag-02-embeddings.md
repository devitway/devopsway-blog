...
Embedding-модель превращает текст в массив чисел. Выбор модели определяет качество всего RAG pipeline. all-MiniLM быстрая, но плохо понимает русский. mxbai-embed-large медленнее, но точнее. Batch-обработка ломается на русском тексте — отправляйте по одному.

---

## Проблема: мусор на входе — мусор на выходе

В [прошлом посте](/posts/rag-01-qdrant-vectors/) мы запустили Qdrant и сделали семантический поиск. Но использовали фейковые вектора (`random.uniform`). В реальном pipeline вектора создаёт embedding-модель — и от неё зависит **всё**.

Плохая модель превращает "настройка reverse proxy" и "проксирование запросов через nginx" в далёкие точки. Хорошая — в соседние. Если модель не понимает русский текст, ваш RAG будет находить ерунду, даже если Qdrant работает идеально.

```
"настройка reverse proxy"
        │
        ▼
 Embedding Model ← ВОТ ЭТО РЕШАЕТ ВСЁ
        │
        ▼
 [0.12, -0.34, 0.56, ..., 0.78]
```

Garbage in — garbage out. Только тут garbage не в данных, а в модели.
...
Нарушение второго правила — самая частая причина "RAG ничего не находит". Переехали на новую модель — переиндексируйте всю базу.
...
| nomic-embed-text | 768 | 274 MB | 8192 tokens | Средне | Быстрая |
| mxbai-embed-large | 1024 | 670 MB | 512 tokens | Хорошо | Средняя |

### Эксперимент: одна фраза, три модели

Проверим, как модели понимают семантическую близость русского текста:
...
Результаты (реальные, не выдуманные):

```
=== all-minilm ===
  0.41  'настройка reverse proxy' ↔ 'проксирование запросов через nginx'
  0.38  'контейнер упал с OOM' ↔ 'процесс убит из-за нехватки памяти'
  0.19  'настройка reverse proxy' ↔ 'рецепт борща'

=== nomic-embed-text ===
  0.67  'настройка reverse proxy' ↔ 'проксирование запросов через nginx'
  0.61  'контейнер упал с OOM' ↔ 'процесс убит из-за нехватки памяти'
  0.14  'настройка reverse proxy' ↔ 'рецепт борща'

=== mxbai-embed-large ===
  0.82  'настройка reverse proxy' ↔ 'проксирование запросов через nginx'
  0.79  'контейнер упал с OOM' ↔ 'процесс убит из-за нехватки памяти'
  0.11  'настройка reverse proxy' ↔ 'рецепт борща'
```

**Разница видна невооружённым глазом:**

- all-MiniLM: score 0.41 для семантически идентичных фраз — это провал. На пороге 0.7 ваш RAG ничего не найдёт.
- nomic-embed-text: 0.67 — лучше, но всё ещё ниже порога. Найдёт не всегда.
- mxbai-embed-large: 0.82 — уверенное совпадение. RAG найдёт правильный фрагмент.

При этом "борщ" везде ниже 0.2 — модели корректно определяют нерелевантный текст.

---

## Подводные камни с русским текстом

### 1. Токенизация: русское слово = 3-5 токенов

Embedding-модели используют tokenizer, обученный преимущественно на английском тексте. Одно английское слово — обычно 1 токен. Одно русское — 3-5:

```
English: "container"     → 1 token  → ["container"]
Русский: "контейнер"     → 4 tokens → ["кон", "тей", "не", "р"]
```

Последствие: русский текст длиной 800 символов может содержать 400+ токенов. Модель с контекстом 256 токенов (all-MiniLM) обрежет его **молча**, потеряв вторую половину.

### 2. Truncation: Ollama truncate=true не работает

Документация Ollama обещает параметр `truncate: true` для автоматической обрезки. На практике — не работает с русским текстом. Модель возвращает ошибку `"context length exceeded"` вместо обрезки.
...
Ollama поддерживает batch embedding — отправить несколько текстов за один запрос. На английском работает. На русском — ломается на длинных текстах: модель молча возвращает пустой массив или ошибку.
...
Да, это медленнее. Но 16,000 чанков индексируются за ~20 минут при поштучной отправке. Для синхронизации по расписанию (re-index только изменённые файлы) — незаметно.
...
| Качество на русском | mxbai-embed-large — хорошо | text-embedding-3-large — отлично |
| Offline | Да | Нет |
| Зависимость | Нет | API key, rate limits, downtime |

Для DevOps-инженера с продакшеном выбор очевиден: **Ollama + mxbai-embed-large**. Данные остаются на вашем сервере. Нет зависимости от внешнего API. Нет счетов за токены. Качество на русском достаточное для RAG.
...
800 символов русского текста ≈ 400+ токенов (одно русское слово = 3-5 токенов). Модель с контекстом 256 обрежет текст примерно на половине. Вектор будет представлять только первую часть текста. Решение: обрезать до 400 символов перед отправкой или использовать модель с большим контекстом.
...
Нет. Для семантически близких фраз score должен быть 0.7+. Score 0.4 означает, что модель плохо понимает язык или домен текста. Нужно сменить модель на более подходящую (для русского — mxbai-embed-large вместо all-MiniLM).
...
Мусорные символы (\ufffd, control chars) и длинные повторяющиеся последовательности (===...===) генерируют шумные вектора, которые "притягивают" нерелевантные результаты при поиске. Quality gate (<20 chars) отсекает слишком короткие фрагменты, которые не несут достаточного смысла для embedding.
...
Чем больше `Separation` (разница между avg similar и avg different) — тем лучше модель различает релевантное от нерелевантного.

---

## Продакшен-параметры

Параметры нашего рабочего pipeline (206,000+ векторов):
...
| CHUNK_SIZE | 800 chars | Safe для 512 tokens с русским markdown |
