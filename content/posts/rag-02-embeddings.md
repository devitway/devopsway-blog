...
Embedding-модель превращает текст в массив чисел. Выбор модели определяет качество всего RAG pipeline. all-MiniLM и nomic-embed-text плохо различают русский текст: борщ и nginx получают одинаковый score. mxbai-embed-large – единственная приемлемая из бесплатных, но требует правильной настройки порога.
...
| nomic-embed-text | 768 | 274 MB | 8192 tokens | Плохо (не различает релевантное от нерелевантного) | Быстрая |
...
Результаты (реальные замеры на нашем сервере, Ollama + RTX 3090):
...
- all-MiniLM: score 0.24 для семантически идентичных фраз – провал. На любом пороге ваш RAG ничего не найдёт. Единственный плюс: борщ (0.15) хотя бы далеко от proxy.
...
**Главный вывод:** ни одна из моделей не даёт на русском тексте score выше 0.8 для семантически идентичных фраз. Для сравнения, на английском те же модели дают 0.85+. Это цена мультиязычности: tokenizer разбивает русское слово на 3-5 частей, и модель "видит" его хуже.
...
Документация Ollama обещает параметр `truncate: true` для автоматической обрезки. На практике – не работает. Модель возвращает HTTP 400: `"the input length exceeds the context length"` вместо обрезки. Проверено на all-MiniLM и mxbai-embed-large с русским текстом.
...
| Качество на русском | mxbai-embed-large – приемлемо (score ~0.72 для похожих фраз) | text-embedding-3-large – значительно лучше |
| Offline | Да | Нет |
...
Зависит от модели и языка. На английском – нет, ожидается 0.7+. На русском – score 0.4 может быть лучшим результатом для all-MiniLM. Проблема в том, что нерелевантный текст (например, "рецепт борща") тоже может получить 0.4 на некоторых моделях (nomic-embed-text). Если separation между релевантным и нерелевантным меньше 0.15 – модель непригодна для вашего языка. Для русского mxbai-embed-large даёт separation ~0.25, что уже рабочий вариант.
