...
cover:
    ...
    caption: "Сначала регистрация ошибки с контекстом, потом детектор на правилах — LLM вне критического пути"
    relative: false
    hidden: false
editPost:
...
Языковые модели полезны, но ставить их барьером между упавшим сервисом и дежурным инженером — преступление. Модель должна стоять сбоку от критического пути.
...
Это единственный уровень, который находится в критическом пути. Здесь всё подчинено правилу: "пришло событие — ушел алерт". Никакого AI.
...
Простейший скелет для критического пути (Tier 0), который не упадет под нагрузкой и не выдумает галлюцинаций:
...
3. **Детерминизм в приоритете.** В критическом пути оповещений не должно быть нейросетей. Только жесткие правила, регулярные выражения и метрики.
