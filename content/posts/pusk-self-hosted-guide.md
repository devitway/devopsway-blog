...
> **Безопасность по умолчанию:** Pusk разрешает создать только одну организацию (`PUSK_MAX_ORGS=1`). Этого хватает для 99% случаев. Если нужно больше — добавь `PUSK_MAX_ORGS=5` (или сколько нужно). Запросы с `PUSK_ADMIN_TOKEN` обходят лимит — в логах будет WARN.
